...
bool D3D11Renderer::Initialize()
{
	...
	pFW1Factory->CreateFontWrapper(this->device, L"Tahoma", &this->pFontWrapper);
...
void D3D11Renderer::DrawLine(float x1, float y1, float x2, float y2, Color color)
{
	if (this->deviceContext == NULL)
	{
		this->swapChain->GetDevice(__uuidof(device), reinterpret_cast<void**>(&device));
